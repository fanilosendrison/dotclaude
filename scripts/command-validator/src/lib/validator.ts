...
export class CommandValidator {
	validate(command: string, _toolName = "Unknown"): ValidationResult {
		...
		// chmod +x → safe, always allow (make file executable)
	...
	containsDangerousCommand(command: string): string | null {
		...
		// Check in chained/substituted commands — only match when in COMMAND POSITION:
		// start of string, after shell separators (; | & \n), or command substitution ($( or `).
		// This avoids false positives on dangerous tokens appearing inside strings,
		// echo arguments, heredoc bodies, paths, or identifiers (e.g. "SR + DD written").
