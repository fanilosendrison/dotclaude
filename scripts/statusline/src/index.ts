...
async function getPermissionMode(projectDir: string): Promise<string | undefined> {
	...
	try {
		const content = await readFile(projectSettings, "utf-8");
		const parsed = JSON.parse(content);
		if (parsed.permissions?.mode) return parsed.permissions.mode;
	} catch {
	...
	try {
		...
		const parsed = JSON.parse(content);
		return parsed.permissions?.mode ?? "default";
	} catch {
		return "default";
	}
}
...
async function main() {
	try {
		...
		const [git, permissionMode] = await Promise.all([
			getGitStatus(),
			getPermissionMode(input.workspace.project_dir),
		]);
